...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        tr_loss1 = torch.tensor(0.0).to(args.device)
        tr_loss2 = torch.tensor(0.0).to(args.device)
            ...
            for _ in range(total_updates):
                ...
                for inputs in batch_samples:
                    ...
                    with self.accelerator.accumulate(model):
                        tr_loss_step, tr_loss_step1, tr_loss_step2 = self.training_step(model, inputs, num_items_in_batch)
                    ...
                    if (
                        ...
                        tr_loss1 = tr_loss1 + tr_loss_step1
                        tr_loss2 = tr_loss2 + tr_loss_step2
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate((tr_loss, tr_loss1, tr_loss2), grad_norm, model, trial, epoch, ignore_keys_for_eval)
                    else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss_tuple, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        tr_loss, tr_loss1, tr_loss2 = tr_loss_tuple
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_loss_scalar1 = self._nested_gather(tr_loss1).mean().item()
            tr_loss_scalar2 = self._nested_gather(tr_loss2).mean().item()
            ...
            logs["loss1"] = round(tr_loss_scalar1 / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["loss2"] = round(tr_loss_scalar2 / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            loss, loss1, loss2 = self.compute_loss(model, inputs, num_items_in_batch=num_items_in_batch)
        ...
        return (loss.detach() / self.args.gradient_accumulation_steps,
                loss1.detach() / self.args.gradient_accumulation_steps,
                loss2.detach() / self.args.gradient_accumulation_steps)
    ...
    def evaluation_loop(
        ...
        all_losses1 = EvalLoopContainer(self.args.eval_do_concat_batches, padding_index=-100)
        all_losses2 = EvalLoopContainer(self.args.eval_do_concat_batches, padding_index=-100)
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            (losses, losses1, losses2), logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            main_input_name = getattr(self.model, "main_input_name", "input_ids")
            inputs_decode = (
            ...
            if losses is not None:
                ...
                losses1 = self.gather_function((losses1.repeat(batch_size)))
                losses2 = self.gather_function((losses2.repeat(batch_size)))
                all_losses.add(losses)
                all_losses1.add(losses1)
                all_losses2.add(losses2)
        ...
        all_losses1 = all_losses1.get_arrays()
        all_losses2 = all_losses2.get_arrays()
        ...
        if isinstance(all_losses, list) and all_losses:
            ...
            metrics[f"{metric_key_prefix}_loss1"] = np.concatenate(all_losses1).mean().item()
            metrics[f"{metric_key_prefix}_loss2"] = np.concatenate(all_losses2).mean().item()
        elif isinstance(all_losses, np.ndarray):
            metrics[f"{metric_key_prefix}_loss"] = all_losses.mean().item()
            metrics[f"{metric_key_prefix}_loss1"] = all_losses1.mean().item()
            metrics[f"{metric_key_prefix}_loss2"] = all_losses2.mean().item()
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                # if has_labels or loss_without_labels:
                with self.compute_loss_context_manager():
                    (loss, loss1, loss2), outputs = self.compute_loss(model, inputs, return_outputs=True)
                loss = loss.mean().detach()
                loss1 = loss1.mean().detach()
                loss2 = loss2.mean().detach()
        ...
        return ((loss, loss1, loss2), logits, labels)
...
class TrainerMCTS(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        Subclass and override for custom behavior.
        """
        visit_weight = inputs.pop('visit_weight')
        batch_rewards_index = inputs.pop('rewards_index')
        batch_rewards_value = inputs.pop('rewards_value')
        batch_rewards_segment = inputs.pop('rewards_segment')
        ...
        # shift_logits = torch.clip(shift_logits, min=0.0, max=1.0)
        # shift_logits = shift_logits / shift_logits.sum(-1, keepdim=True).clamp(min=1e-8)
        shift_probs = torch.log_softmax(shift_logits, dim=-1)

        row_indices = torch.repeat_interleave(torch.arange(len(batch_rewards_segment), device=shift_logits.device), batch_rewards_segment)
        extracted_probs = shift_probs[row_indices, batch_rewards_index]

        # visit_weight_expand = torch.repeat_interleave(visit_weight, batch_rewards_segment)

        # extracted_logits = extracted_logits.clamp(min=1e-12)
        loss1 = -torch.mean(batch_rewards_value * extracted_probs)
        # loss = F.mse_loss(extracted_logits, batch_rewards_value, reduction='none')

        # loss3 = torch.zeros_like(shift_logits)
        # loss3[row_indices, batch_rewards_index] = loss
        # loss3 = loss3.sum(-1)

        # loss1 = loss3 * visit_weight
        # loss1 = loss1.mean()
...
class TrainerValid(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        torch.sigmoid_(shift_logits)
        shift_probs = shift_logits.clamp(min=1e-7, max=1.0-1e-7)

        row_indices = torch.repeat_interleave(torch.arange(len(batch_rewards_segment), device=shift_logits.device), batch_rewards_segment)
        # extracted_probs = shift_probs[row_indices, batch_rewards_index]


        mask = torch.zeros_like(shift_probs, dtype=torch.bool)
        mask[row_indices, batch_rewards_index] = True

        shift_probs_positive = shift_probs
        shift_probs_negetive = shift_probs.clone()

        shift_probs_positive[mask] = -torch.log(shift_probs_positive[mask])
        shift_probs_positive[~mask] = 0

        shift_probs_negetive[~mask] = -torch.log(1 - shift_probs_negetive[~mask])
        shift_probs_negetive[mask] = 0

        loss3 = shift_probs_positive.sum(-1) * (1-batch_rewards_segment/shift_logits.shape[-1])
        loss3 += shift_probs_negetive.sum(-1) * batch_rewards_segment/shift_logits.shape[-1]





        # positive_probs = shift_probs[row_indices, batch_rewards_index]
        # shift_probs = -torch.log(1-shift_probs)
        # shift_probs[row_indices, batch_rewards_index] = -torch.log(positive_probs)
        
        # mask = torch.zeros_like(shift_probs, dtype=torch.bool)
        # mask[row_indices, batch_rewards_index] = True
        # shift_probs[mask] = -torch.log(shift_probs[mask])  # 目标位置为 log(p)
        # shift_probs[~mask] = -torch.log(1 - shift_probs[~mask]) 
        

        # visit_weight_expand = torch.repeat_interleave(visit_weight, batch_rewards_segment)

        # extracted_logits = extracted_logits.clamp(min=1e-12)
        # loss1 = -torch.mean(visit_weight_expand * torch.log(extracted_probs))
        # loss = F.mse_loss(extracted_logits, batch_rewards_value, reduction='none')

        # loss3 = torch.zeros_like(shift_logits)
        # loss3[row_indices, batch_rewards_index] = loss
        # loss3 = shift_probs.sum(-1)
