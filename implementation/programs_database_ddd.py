...
class ProgramsDatabase:
    ...
    def __init__(
        ...
        self._parent_visit_count = -1  # one slot for root node
    ...
    def get_prompt(self) -> Prompt:
        ...
        return Prompt(code, version_generated, parent_id)
    ...
    def register_program(
        ...
        self._parent_visit_count += 1
        for id in parent_id:
            parent_node = self._nodes[id]
            parent_node.visit_count += 1

        self._parent_id_dic[kwargs.get('global_sample_nums', None)] = parent_id

        if scores_per_test:
            score = _reduce_score(scores_per_test)

            node = Node(visit_count=0, score=score, score_update=score, parent_id=parent_id, node_id=self._node_id, program=program)
            ...
            with open('nodes.pkl', 'wb') as f:
                pickle.dump((self._nodes, self._parent_id_dic), f)
